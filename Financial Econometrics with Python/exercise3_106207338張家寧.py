...
reg_educ = smf.ols('educ ~ exper + tenure', data = wage1)
...
print(f'beta_educ_hat1: \n{beta_educ_hat1}\n')          # should be the same as beta_educ
...
reg_int = smf.ols(formula = ' ones ~ educ + exper + tenure - 1', data = wage1)     # note the formula, there is '-1' here!
results_int = reg_int.fit()
wage1['resid_int'] = results_int.resid

# The second step: regress y on resid_educ with the OLS 
# and obtain the estimated slop parameter
reg_int1 = smf.ols(formula = 'np.log(wage) ~ resid_educ - 1', data = wage1)
results_int1 = reg_int1.fit()
beta_int_hat1 = results_int1.params['resid_educ']
beta_int_hat1 = round(beta_int_hat1, 4)

# The OLS intercept from original regression
beta_int_hat = results.params['educ']
...
# e. Does the result apply to the intercept term? Yes
delta_int_hat = results_IQ.params['Intercept']
print(round(delta_int_hat, 4))

beta_int_tilde = results1.params['Intercept']
print(round(beta_int_tilde, 4))

beta_int_tilde1 = beta_hat['Intercept']+beta_hat['IQ']*delta_int_hat
...
# a. Import data, transfer DATE to date, 
# calculate simple return and risk premium
data = pd.read_csv("C:/Users/Luc/Desktop/用python學計量/data_exercise_3.csv")
...
data['AAPL_sr'] = np.append(np.nan, AAPL_sr*100)
...
# Note the trick on using loc and column names 
results_sst = data.loc[:,'Mkt_RF':'AAPL_rp'].describe()
print(results_sst)

# d. Fit the CAPM
data = data.drop(0)      # remove the row containing NaN
